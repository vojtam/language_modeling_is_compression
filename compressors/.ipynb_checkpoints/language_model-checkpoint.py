...
DNA_INDEX_MAP = {index: base for index, base in enumerate('ACGT')}
...
def _retrieve_predict_fn():
    ...
    def predict_fn(x, compressing = True) -> np.ndarray:
        if compressing:
            seq = "".join(x[0])
        else:

            seq = "".join([DNA_INDEX_MAP[i] for i in x[0]])
        # Tokenize the input
...
def compress(
  ...
  if use_slow_lossless_compression:
    ...
    for subsequence_length, symbol in enumerate(dna_seq):
      ...
      probs.append(list(subsequence_probs.values()))
    probs = np.vstack(probs)
  else:
    probs = predict_fn(dna_seq[None])[0, ...]
  ...
  dna_base_index = {base: index for index, base in enumerate('ACGT')}
  for pdf, symbol in zip(probs, dna_seq):
    symbol_index = dna_base_index[symbol]
    encoder.encode(utils.normalize_pdf_for_arithmetic_coding(pdf), symbol_index)
  encoder.terminate()
  print(output)
...
def decompress(
  ...
  probs = np.array([list(predict_fn(sequence_array[None], False).values())])

  for _ in range(uncompressed_length):
  ...
  print("".join(DNA_INDEX_MAP[i] for i in sequence_array[:-1]))
